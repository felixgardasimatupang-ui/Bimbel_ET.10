...
export default function AuditLogPanel() {
  ...
  // Real-time subscription via Supabase Realtime (deferred, timeout 5s)
  useEffect(() => {
    const sb = supabase;
    if (!sb) return;
  ...
  return (
    <div id="panel_audit" className="space-y-3 flex flex-col flex-1">
      ...
      {/* Table */}
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm flex-1 overflow-hidden flex flex-col">
                ...
                return (
                  <tr
                    ...
                    className={`hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-all duration-150 ${
                      isLive ? 'bg-emerald-50/50 dark:bg-emerald-900/20 border-l-2 border-l-emerald-500 animate-slideIn' : ''
                    }`}
                  >
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-2 shadow-sm flex items-center justify-between text-[9px] text-slate-400">
        <div className="flex items-center gap-2">
